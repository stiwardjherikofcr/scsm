...
CREATE TABLE IF NOT EXISTS `scsm`.`tabla_info` (
  ...
  INDEX `fk_tabla_info_tabla_seccion1_idx` (`tabla_seccion_seccion_microcurriculo_id` ASC),
  ...
  CONSTRAINT `fk_tabla_info_tabla_seccion1`
    FOREIGN KEY (`tabla_seccion_seccion_microcurriculo_id`)
    REFERENCES `scsm`.`tabla_seccion` (`seccion_microcurriculo_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
